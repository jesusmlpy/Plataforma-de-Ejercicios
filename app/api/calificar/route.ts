...
export async function POST(req: NextRequest) {
  const { enunciado, respuestaAlumno } = await req.json();
...
Evalúa si el razonamiento es correcto (no hace falta que use exactamente las mismas palabras que un libro, solo que la idea matemática sea válida). Responde ÚNICAMENTE con este JSON, sin texto adicional ni backticks:
